...
create_ash_script() {
    ...
    if [[ ! -f "$zsh_source" && ! -f "$bash_source" ]]; then
        print_error "No ash source files found"
        print_info "Please ensure 'ash.zsh' and/or 'ash.sh' are in the same directory as the installer"
        return 1
    fi
}
